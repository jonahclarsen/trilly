...
test('Command-Z restores a skipped item before undoing the preceding approval', async ({ page }) => {
  ...
  await page.keyboard.press('Meta+z')
  await expect.poll(() => actions.filter(a => a.action === 'undo').length).toBe(1)
})
...
test('Undo retraces consecutive skips even with no approvals', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Brew House', exact: true })).toBeVisible()
  await page.keyboard.press('Meta+z')
  await expect(page.locator('.transaction h1')).toHaveText(first!)
...
test('Undo reverses an approval before an earlier skip and preserves that skip', async ({ page }) => {
  ...
  await page.keyboard.press('Meta+z')
  await expect(page.getByRole('heading', { name: 'Brew House', exact: true })).toBeVisible()
  await expect(page.getByRole('button', { name: '1 skipped', exact: true })).toBeVisible()
  await page.keyboard.press('Meta+z')
  await expect(page.locator('.transaction h1')).toHaveText(first!)
  expect(actions.filter(a => a.action === 'undo')).toHaveLength(1)
})
...
test('logo controls persist and token replacement stays hidden until requested', async ({ page }) => {
  ...
  const reset = page.getByRole('button', { name: 'Reset logo', exact: true })
  await expect(reset).toBeDisabled()
  const preview = page.locator('.logo-preview > .brand')
  await expect(preview).toHaveText('trilly')
  await expect(preview).toBeVisible()
  await expect(page.getByRole('slider', { name: 'Logo weight', exact: true })).toHaveCount(0)
  await expect(preview).toHaveCSS('font-weight', '750')
  await page.getByRole('textbox', { name: 'Logo hex color' }).fill('#aa3377')
  await expect(reset).toBeEnabled()
  await page.getByRole('textbox', { name: 'Logo hex color' }).fill('#56C2F0')
  await expect(reset).toBeDisabled()
  await page.getByRole('textbox', { name: 'Logo hex color' }).fill('#aa3377')
  await expect(preview).toHaveCSS('color', 'rgb(170, 51, 119)')
  ...
  await expect(page.locator('header > .brand')).toHaveCSS('font-weight', '750')
  await expect(preview).toHaveCSS('font-weight', '750')
  await expect(preview).toHaveCSS('color', 'rgb(86, 194, 240)')
})
...
test('business expense keyboard entry, editing, copy, reload, archive and undo', async ({ page, context }) => {
  ...
  await page.keyboard.press('Enter')
  await expect(page.getByRole('dialog')).toHaveCount(0)
  await expect(page.getByRole('button', { name: 'Business expense B' })).toBeEnabled()
  ...
  await page.keyboard.press('b')
  await expect(page.getByRole('dialog', { name: 'Edit business expense', exact: true })).toBeVisible()
  await expect(page.getByLabel('Description', { exact: true })).toHaveValue('Office supplies')
  await page.keyboard.press('Escape')
  await page.reload()
  await page.getByRole('button', { name: 'Business expenses (1)', exact: true }).click()
  await expect(page.getByLabel('Description for 2026-09-08')).toHaveValue('Office supplies')
  ...
  await expect(page.getByLabel('Description for 2026-09-08')).toHaveValue('Office equipment')
  await page.getByRole('button', { name: 'Remove expense: Office equipment', exact: true }).click()
  await expect(page.getByLabel('Description for 2026-09-08')).toHaveCount(0)
  await page.keyboard.press('u')
  await expect(page.getByLabel('Description for 2026-09-08')).toHaveValue('Office equipment')
  await page.keyboard.press('Escape')
  await page.keyboard.press('u')
