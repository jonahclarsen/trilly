...
test('logo controls persist and token replacement stays hidden until requested', async ({ page }) => {
  ...
  await expect(page.getByLabel('Replace access token', { exact: true })).toHaveCount(0)
  await expect(page.locator('.font-select')).toHaveCount(10)
  const families = await page.locator('.font-select .brand').evaluateAll(nodes => nodes.map(node => getComputedStyle(node).fontFamily))
  expect(new Set(families).size).toBe(10)
  await page.getByRole('button', { name: 'Avenir Next', exact: true }).click()
  await page.getByRole('button', { name: 'Semibold', exact: true }).click()
  await page.getByRole('slider', { name: /Letter spacing/ }).focus()
  ...
  await expect(page.locator('header > .brand')).toHaveCSS('font-weight', '600')
  await expect(page.locator('header > .brand')).toHaveCSS('letter-spacing', '-1.9px')
...
test('business expense keyboard entry, copy, reload, archive and undo', async ({ page, context }) => {
  ...
  await expect(page.getByRole('button', { name: 'Undo archive', exact: true })).toBeVisible()
  await page.reload()
